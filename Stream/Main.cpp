...
const int FPS = 60;
const int COLOR_FILTER = 6;
const int MAX_MARGIN = 4;
const double CONTRAST_JUMP = 0.25;
const double FACE_MIN = 1.3;
const double FACE_MAX = 3;
...
int main()
{
	...
	cv::Mat fframe; //face frame
	cap.read(nframe);
	
	//face detection model
	cv::CascadeClassifier faceCascade;
	faceCascade.load("haarcascade_frontalcatface.xml");
	SDL_Rect Frect = {IMAGE_WIDTH, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_HEIGHT};
	...
	while(run)
	{
				...
				case SDL_KEYDOWN:
					switch(e.key.keysym.sym)
					{
						...
						case SDLK_c:
							...
							break;
						case SDLK_v:
							if(contrastThresh < 10.0)
								contrastThresh += CONTRAST_JUMP;
							break;
						case SDLK_b:
		...
		{
			//detect faces
			cv::cvtColor(frame, fframe, cv::COLOR_BGR2GRAY);
			std::vector<cv::Rect> faces;
			faceCascade.detectMultiScale(fframe, faces, FACE_MIN, FACE_MAX);
			for(const auto&face:faces)
			{
				//diplay result
				cv::rectangle(frame, face, cv::Scalar(0, 0, 0), 2);
		...
		std::string str2 = std::to_string(contrastThresh).substr(0, 4);
		...
		if(flipc == 0)
			...
			drawText(20, thr_rect.y+250, font, "Face Detection");
		if(contrast)
